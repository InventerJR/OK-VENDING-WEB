...
type ContextInterface = {
    ...
    handleConfirmLoad: (loadData: any) => Promise<void>; // Añadir esta línea
};
...
export const CartContextProvider = ({ children }: ProviderProps) => {
    ...
    const value: ContextInterface = {
        ...
        openCartModal,
        closeTicketCart,
        handleConfirmLoad, // Añadir esta línea
    };

    return (
        <Context.Provider value={value}>
            <div className='relative w-full'>
                {isOpenCartModal && <CartModalView isOpen={isOpenCartModal} onClose={closeCart} origin={''} destination={''} cash={''} />}
                {children}
            </div>
        </Context.Provider>
    );
